...
public class DorisDatabaseDialect implements DatabaseDialect {
    ...
    public DorisDatabaseDialect(SqlClient sqlClient) {
    ...
    public QueryResult listSchemas() {
        return sqlClient.query("""
            SELECT
                ...
                CASE
                    WHEN SCHEMA_NAME IN ('mysql', 'information_schema', 'performance_schema', 'sys', '__internal_schema')
                        THEN 'System Schema'
                    ELSE 'User Schema'
                END AS schema_type
    ...
    public QueryResult listObjects(String schemaName, String objectType) {
        return switch (objectType) {
            ...
            case "sequence" -> throw new UnsupportedOperationException(
                "Doris 不支持 PostgreSQL 风格的 sequence 对象。"
            );
            case "extension" -> throw new UnsupportedOperationException(
                "Doris 不支持 PostgreSQL 风格的 extension 对象。"
            );
    ...
    public QueryResult getObjectDetails(String schemaName, String objectName, String objectType) {
        return switch (objectType) {
            case "table" -> getTableOrViewDetails(schemaName, objectName, "BASE TABLE");
            case "view" -> getTableOrViewDetails(schemaName, objectName, "VIEW");
            case "sequence" -> throw new UnsupportedOperationException(
                "Doris 不支持 PostgreSQL 风格的 sequence 对象。"
            );
            case "extension" -> throw new UnsupportedOperationException(
    ...
    public QueryResult explain(String sql) {
        ...
        try {
            ...
            throw new RuntimeException(SecretMasker.mask(e.getMessage()), e);
        }
    }

    private QueryResult getTableOrViewDetails(String schemaName, String objectName, String tableType) {
        try {
            return sqlClient.query("""
                SELECT
                    ? AS schema_name,
                ...
                WHERE t.TABLE_SCHEMA = ? AND t.TABLE_NAME = ? AND t.TABLE_TYPE = ?
                """, List.of(schemaName, objectName, schemaName, objectName, tableType));
        } catch (RuntimeException e) {
            return degradedDetails(schemaName, objectName, e);
        }
    }

    private QueryResult degradedDetails(String schemaName, String objectName, RuntimeException e) {
        return new QueryResult(List.of(Map.of(
            "schema_name", schemaName,
            "object_name", objectName,
            "columns", "[]",
            "constraints", "[]",
            "indexes", "{\"indexes\":\"当前 Doris 版本或权限无法获取索引统计：" + SecretMasker.mask(e.getMessage()) + "\"}"
        )));
    }
}
